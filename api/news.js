export default async function handler(req, res) {
  ...
  try {
    ...
    results.naver = (naverData.items || []).map(function(i) {
      return {
        title: i.title.replace(/<[^>]+>/g, ''),
        url: i.link,
        source: (function() { try { return new URL(i.originallink).hostname.replace('www.',''); } catch(e) { return 'naver'; } })(),
  ...
  try {
    ...
    const items = Array.from(rssText.matchAll(/<item>([\s\S]*?)<\/item>/g)).slice(0, 5);
    results.google = items.map(function(m) {
      const block = m[1];
      function getTag(tag) {
      ...
      const source = match ? match[2].trim() : 'Google';
      const pubDate = getTag('pubDate');
      const time = pubDate ? new Date(pubDate).toLocaleDateString('ko-KR', { month: 'numeric', day: 'numeric' }) : '';
      const link = getTag('link') || '#';
      return { title: title, source: source, time: time, url: link, from: 'g' };
    });
  } catch (e) {
